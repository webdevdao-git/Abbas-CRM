...
export default function RsvpButtons({ value, onChange, busy = false, compact = false }) {
  return (
    <div className="inline-flex rounded-xl border border-ink-200 p-0.5" role="group" aria-label="Set RSVP status">
      {OPTIONS.map((option) => {
        ...
        return (
          <button
            ...
            title={option.label}
            className={`inline-flex items-center gap-1 rounded-[10px] border border-transparent px-2.5 py-1.5 text-xs font-semibold transition-colors disabled:opacity-50 ${
